...
import { modalMoved, storyDetailSaved, boardTitleSaved} from '../../actions';
...
const UpdateBoard = (props) => {
    ...
    const { boardIndex, storyDetail, boardTitle } = state.pageReducer;
    const history = useHistory();
    const [text, setText] = 
        useState(EditorState.createWithContent(ContentState.createFromBlockArray(htmlToDraft(storyDetail).contentBlocks)))

    const onEditorStateChange = (text) => {
        setText(text);              
        if(text.getCurrentContent().getPlainText()) {
            dispatch(storyDetailSaved(draftToHtml(convertToRaw(text.getCurrentContent()))));
        }
        else {
            dispatch(storyDetailSaved(''))
        }
    }

    const handleTitle = (e) => {
        dispatch(boardTitleSaved(e.target.value))
    }
    ...
    return (
        ...
        <EditorStyle>
            <div className = "title">
                <div>Title </div>
                <input placeholder="Please enter a title" value = {boardTitle} onChange = {handleTitle}/>
            </div>
            <Editor
                editorState={text}
